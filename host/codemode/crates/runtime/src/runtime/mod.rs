...
use wuu_codemode_shim::ToolName;
...
#[cfg(test)]
mod concurrent_isolate_probe {
    use std::collections::HashMap;
    use std::sync::{Arc, Barrier};
    use tokio::sync::mpsc;

    #[test]
    fn concurrent_first_isolates_do_not_trap() {
        crate::v8_init::ensure_v8_initialized().unwrap();
    ...
    #[test]
    fn concurrent_spawn_runtime_does_not_trap() {
        let barrier = Arc::new(Barrier::new(2));
        let mut handles = Vec::new();
        for _ in 0..2 {
            let b = barrier.clone();
            handles.push(std::thread::spawn(move || {
                b.wait();
                let (event_tx, mut event_rx) = mpsc::unbounded_channel();
                let (_tx, _ctrl, handle) = super::spawn_runtime(
                    HashMap::new(),
                    ...
                    event_tx,
                    super::PendingRuntimeMode::Continue,
                    None,
                )
                .unwrap();
                // Wait for a result or termination; the runtime thread owns the isolate.
