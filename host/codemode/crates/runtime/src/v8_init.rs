...
fn initialize_v8_with_mode(jit_mode: V8JitMode) -> Result<V8Initialization, String> {
    ...
    v8::V8::initialize();
    // On macOS with the V8 sandbox enabled, the partition-alloc configurable
    // pool is initialized by the first `IsolateGroup` creation with a
    // check-then-init race: two isolates created concurrently at process
    // startup both observe the pool uninitialized, and the loser dies with
    // `[FATAL:address_pool_manager.cc(62)] Check failed:
    // !pool->IsInitialized()`. Creating (and dropping) one isolate here, under
    // the process-wide `OnceLock`, initializes the pool before any cell
    // runtime can spawn, so concurrent `Isolate::new` calls afterward take the
    // already-initialized path. See `concurrent_first_isolates_do_not_trap`.
