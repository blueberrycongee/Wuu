...
func (b BuildInfo) String() string {
	...
	if b.Version == "dev" {
		...
		if b.Date == "" || b.Date == "unknown" {
			return fmt.Sprintf("wuu dev (%s)", commit)
		}
		return fmt.Sprintf("wuu dev (%s %s)", commit, b.Date)
	}

	if b.Date == "" || b.Date == "unknown" {
		return fmt.Sprintf("wuu %s (%s)", b.Version, commit)
	}
	return fmt.Sprintf("wuu %s (%s %s)", b.Version, commit, b.Date)
}
