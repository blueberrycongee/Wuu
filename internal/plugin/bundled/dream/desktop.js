export async function activate(api) {
  ...
  api.registerStyle({ id: "dream-settings", css: `
    ...
    .plugin-dream-rows { display:flex; flex-direction:column; }
    .plugin-dream-row { display:grid; grid-template-columns:minmax(0,1fr) auto; align-items:center; gap:24px; padding:10px 0; border-bottom:1px solid var(--hairline-soft,var(--hairline)); }
    .plugin-dream-row:first-child { padding-top:2px; }
    ...
    .plugin-dream-num { width:104px; text-align:right; font-variant-numeric:tabular-nums; }
    .plugin-dream-model { width:200px; }
    ...
    .plugin-dream-stats { display:flex; flex-direction:column; }
    .plugin-dream-stat { display:grid; grid-template-columns:minmax(0,1fr) auto; align-items:center; gap:24px; padding:8px 0; border-bottom:1px solid var(--hairline-soft,var(--hairline)); }
    ...
    .plugin-dream-stat-value { color:var(--wuu-color-text,var(--ink)); font-size:var(--font-ui,13px); font-weight:var(--weight-medium,500); font-variant-numeric:tabular-nums; text-align:right; }
  ...
  function DreamSettings(props) {
    ...
    if (!draft) return h(Page, { className: "plugin-dream" }, h("div", { className: "plugin-dream-loading" }, "…"));
      ...
      h(Section, { title: tr("dream.activity") }, h("div", { className: "plugin-dream-stats" },
        ...
        h("div", { className: "plugin-dream-stat" },
          ...
          h("span", { className: "plugin-dream-stat-value" },
            ...
            `${status}${state?.last_error ? ` · ${state.last_error}` : ""}`))))
    ));
  }
