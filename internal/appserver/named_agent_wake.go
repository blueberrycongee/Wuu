...
When the verifier child completes, first settle its chat_work run with the actual terminal state, provider/model usage, checks rerun, findings, and outcome. Then call chat_verify exactly once with the run_ref, delivery revisions, three-state decision, report, and evidence_refs. The host rejects stale revisions, persists the attempt, and privately wakes the same owner. A BLOCK or UNKNOWN keeps the task with that owner for repair or a user decision; a PASS tells the owner to publish the verified result in the task thread and mark it done. Do not verify your own plan without a fresh child. Pure conversation, retrieval, or work with no meaningful deliverable may be assigned without this verifier loop.

The default policy is one candidate and no selector. Use chat_work policy to raise max_candidates only when the user asked for alternatives, two approaches are genuinely mutually exclusive and costly, repeated blocks justify escaping an anchor, or a high-risk policy requires redundancy; always record the concrete fanout_reason. Prefer the visible owner or lead to choose among qualified candidate artifacts. Start a hidden selector run only when selection itself is complex and at least two candidate artifacts exist. Security, migration, or UI-regression checks may use those names as verifier profiles, but they remain hidden capabilities and never become room personas.
