...
func TestThreadGoalProtocolLifecycleAndNotifications(t *testing.T) {
	rt := newTestRuntime(t, &fakeClient{})
	out := &lockedBuffer{}
	srv := New(rt, out)
	...
	getRaw := `{"id":"get-empty","method":"thread/goal/get","params":{"thread_id":` + quoteGoalHandlerJSON(threadID) + `}}`
	if err := srv.handleLine(context.Background(), []byte(getRaw)); err != nil {
		t.Fatalf("thread/goal/get empty: %v", err)
	}
	empty := remarshal[ThreadGoalGetResult](t, responseByID(t, parseOutput(t, out.String()), "get-empty")["result"])
	if empty.Goal != nil {
		t.Fatalf("empty Goal = %+v, want nil", empty.Goal)
	}

	setRaw := `{"id":"set","method":"thread/goal/set","params":{"thread_id":` + quoteGoalHandlerJSON(threadID) + `,"objective":"ship thread Goal protocol"}}`
	if err := srv.handleLine(context.Background(), []byte(setRaw)); err != nil {
		t.Fatalf("thread/goal/set: %v", err)
	}
	setResult := remarshal[ThreadGoalSetResult](t, responseByID(t, parseOutput(t, out.String()), "set")["result"])
	if setResult.Goal.ThreadID != threadID || setResult.Goal.Objective != "ship thread Goal protocol" || setResult.Goal.Status != "active" {
		t.Fatalf("unexpected set Goal: %+v", setResult.Goal)
	}
	if setResult.Goal.CreatedAt == 0 || setResult.Goal.UpdatedAt == 0 {
		t.Fatalf("Goal timestamps should be unix seconds: %+v", setResult.Goal)
	}
	messages := parseOutput(t, out.String())
	updates := notificationsByMethod(messages, NotificationThreadGoalUpdated)
	if len(updates) == 0 {
		t.Fatalf("missing %s notification: %+v", NotificationThreadGoalUpdated, messages)
	}
	updated := updates[len(updates)-1]
	updatedParams := remarshal[ThreadGoalUpdatedNotification](t, updated["params"])
	if updatedParams.ThreadID != threadID || updatedParams.TurnID != "" || updatedParams.Goal.Objective != setResult.Goal.Objective {
		t.Fatalf("unexpected Goal update notification: %+v", updatedParams)
	}
	encoded, err := json.Marshal(setResult.Goal)
	if err != nil {
		t.Fatalf("marshal Goal: %v", err)
	}
	for _, forbidden := range []string{"goal_id", "token_budget", "usage_limit", "budget_limit"} {
		if strings.Contains(string(encoded), forbidden) {
			t.Fatalf("public Goal leaked %q: %s", forbidden, encoded)
		}
	}

	paused := "paused"
	pauseParams, _ := json.Marshal(ThreadGoalSetParams{ThreadID: threadID, Status: &paused})
	pauseRequest, _ := json.Marshal(map[string]any{"id": "pause", "method": MethodThreadGoalSet, "params": json.RawMessage(pauseParams)})
	if err := srv.handleLine(context.Background(), pauseRequest); err != nil {
		t.Fatalf("pause Goal: %v", err)
	}
	pauseResult := remarshal[ThreadGoalSetResult](t, responseByID(t, parseOutput(t, out.String()), "pause")["result"])
	if pauseResult.Goal.Status != "paused" {
		t.Fatalf("paused Goal = %+v", pauseResult.Goal)
	}

	clearRaw := `{"id":"clear","method":"thread/goal/clear","params":{"thread_id":` + quoteGoalHandlerJSON(threadID) + `}}`
	if err := srv.handleLine(context.Background(), []byte(clearRaw)); err != nil {
		t.Fatalf("thread/goal/clear: %v", err)
	}
	clearResult := remarshal[ThreadGoalClearResult](t, responseByID(t, parseOutput(t, out.String()), "clear")["result"])
	if !clearResult.Cleared {
		t.Fatalf("clear result = %+v", clearResult)
	}
	clearedNotifications := notificationsByMethod(parseOutput(t, out.String()), NotificationThreadGoalCleared)
	if len(clearedNotifications) == 0 {
		t.Fatal("missing Goal cleared notification")
	}
	cleared := clearedNotifications[len(clearedNotifications)-1]
	if remarshal[ThreadGoalClearedNotification](t, cleared["params"]).ThreadID != threadID {
		t.Fatalf("missing Goal cleared notification: %+v", cleared)
	}
}
