...
func TestQueuedSpawnCancellationTombstoneRecoversHelpMeRollbackWithoutLaunch(t *testing.T) {
	rootDir := t.TempDir()
	harnessDir := filepath.Join(rootDir, "harness")
	const workerID = "worker_cancelled_helpme_recovery"
	persistQueuedSpawnOfTypeForConcurrencyTest(t, harnessDir, workerID, HelpMeRecoveryWorkerType, time.Now().UTC())

	seed := newDormantQueuedSpawnConcurrencyControl(
		t,
		rootDir,
		filepath.Join(rootDir, "seed-control"),
		harnessDir,
		1,
		&queuedSpawnCountingClient{},
		nil,
	)
	if err := seed.RegisterHelpMeRecovery(HelpMeRecovery{
		HelperID: workerID,
		Brief:    HelpMeRecoveryBrief{OriginalGoal: "recover cancellation safely"},
	}); err != nil {
		t.Fatalf("register HelpMe recovery: %v", err)
	}
	if marked, err := seed.HarnessStore().MarkQueueItemCancelling(workerID); err != nil || !marked {
		t.Fatalf("mark durable cancellation tombstone = %v, %v; want true, nil", marked, err)
	}
	...
	if recovered.Manager().Get(workerID) != nil {
		t.Fatal("cancelled HelpMe tombstone reached Manager.Spawn")
	}
}

func TestQueuedSpawnFailureTombstoneRecoversHelpMeRollbackWithoutLaunch(t *testing.T) {
	rootDir := t.TempDir()
	harnessDir := filepath.Join(rootDir, "harness")
	const workerID = "worker_failed_helpme_recovery"
	const failureText = "worker toolkit failed before process exit"
	persistQueuedSpawnOfTypeForConcurrencyTest(t, harnessDir, workerID, HelpMeRecoveryWorkerType, time.Now().UTC())

	seed := newDormantQueuedSpawnConcurrencyControl(
		t,
		rootDir,
		filepath.Join(rootDir, "seed-failure-control"),
		harnessDir,
		1,
		&queuedSpawnCountingClient{},
		nil,
	...
	if err := seed.RegisterHelpMeRecovery(HelpMeRecovery{
		HelperID: workerID,
		Brief:    HelpMeRecoveryBrief{OriginalGoal: "recover launch failure safely"},
	}); err != nil {
		t.Fatalf("register HelpMe recovery: %v", err)
	}
	if marked, err := seed.HarnessStore().MarkQueueItemFailing(workerID, failureText); err != nil || !marked {
		t.Fatalf("mark durable failure tombstone = %v, %v; want true, nil", marked, err)
	}
	...
	recovered := newQueuedSpawnConcurrencyControl(
		t,
		rootDir,
		filepath.Join(rootDir, "recovered-failure-control"),
		harnessDir,
		1,
		client,
		nil,
	...
	waitForQueuedSpawnRecoveryTest(t, func() bool {
		...
		return existsErr == nil && !exists && !recoveryExists && metaExists &&
			meta.Status == agentthread.StatusFailed && meta.Source.EdgeStatus == agentthread.EdgeClosed
	}, "recovered failure tombstone to compensate and settle")
	if got := client.calls.Load(); got != 0 {
		t.Fatalf("failed HelpMe provider executions = %d, want 0", got)
	}
	if recovered.Manager().Get(workerID) != nil {
		t.Fatal("failed HelpMe tombstone reached Manager.Spawn")
	}
}
