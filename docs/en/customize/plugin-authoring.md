...
appearance plugin does not need private DOM: `--wuu-control-secondary-background`
controls secondary actions, `--wuu-control-field-background` controls text
fields and selects, `--wuu-control-icon-background` controls compact icon
tiles, `--wuu-badge-neutral-background` controls neutral status and permission
badges, and `--wuu-inline-code-background` controls Markdown inline code. Text
and fonts continue to inherit the corresponding public color and typography
tokens. Compact controls keep host-owned border widths so a strong panel
border cannot change their box model or break text wrapping.
