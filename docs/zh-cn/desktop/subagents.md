...
主 Agent 通过每次任务的 `run_in_background` 选择模式，默认在后台运行。当下一步依赖子代理
结果时，主 Agent 将它设为 `false`；当前工具调用会持续等待，并直接取得子代理的终态、最终输出
和错误。前台等待超过十分钟会自动转为后台，避免缓慢或卡住的子代理无限阻塞父会话。

Subagent 插件在 Composer 上方贡献当前父会话的子任务状态。后台任务的完成交付显示成普通只读
query 气泡；气泡只展示“子任务 A 已更新”一类摘要，完整交接 Prompt 只进入模型输入。前台任务
通过原工具调用返回，不会再产生重复的完成气泡。持久记录仍保留插件来源和 cause。禁用插件后，
状态 Slot、Tool、Prompt 和完成订阅会一起撤回。

停止主会话当前回复不会伪装成对子 Session 的产品专用控制。需要终止子任务时，主 Agent 使用
Subagent 插件的 `close_agent`，由公共 Session cancel 执行；需要追加要求时使用 `send_message`。
...
| [Automations](../automation/scheduled-tasks.md) | 在未来时间按计划启动任务 | Automation 插件 Timer 届时向 Session 投递普通 query | 单次执行结束，或重复任务暂停、删除 |
