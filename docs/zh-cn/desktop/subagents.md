...
Subagent 插件在 Composer 上方贡献当前父会话的子任务状态，并把完成交付显示成普通只读
query 气泡。气泡只展示“子任务 A 已更新”一类摘要；完整交接 Prompt 只进入模型输入，持久记录
仍保留插件来源和 cause。禁用插件后，状态 Slot、Tool、Prompt 和完成订阅会一起撤回。
...
- 当前桌面体验以“主 Agent 自动判断是否委派”为主，暂不提供通过 `@agent` 手动点名子代理
  或切换主 Agent 的交互。
- Subagent 插件在 Composer 工具栏提供 A+ 主动委派开关；它只改变后续模型请求的插件 Prompt，
  不在核心保存模式或修改正在执行的 Turn。
