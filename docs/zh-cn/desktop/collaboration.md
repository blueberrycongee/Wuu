...
每个 Named Agent 最多有五个已经准入（启动中或运行中）的 Work session；Room 和整个应用也有
独立上限。超过上限的 Run 会持久排队，不会被丢弃或重复创建。完成、取消、中断或超时会释放
槽位并继续准入最早的合法排队 Run；历史和空闲 session 不占这五个槽位。
...
困难且适合并行的 Work 可以拥有多个目标不同的 Producer Run。每条路线的候选 Artifact 分别
保存，普通 Producer 完成不会覆盖正式候选。只有单候选流程，或可见 Lead Named Agent 的
Selector / Integration Run，才能显式提升一个 Artifact 为正式候选。提升是原子且带版本校验的；
只有提升后的候选会进入 checking 和独立验证。Run 的完成、失败、取消、中断和超时都会形成
持久终态事件，Room Agent 据此决定继续下一波、在部分失败后降级、整合、停止或请求用户输入。
