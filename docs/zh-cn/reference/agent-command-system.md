...
当你让 Wuu “跑一下测试”“启动开发服务器”或“执行这个脚本”时，Agent 不会直接把一串文字扔给系统后就不管了。Wuu 会判断这是一条很快结束的命令、一个需要持续运行的任务，还是一个需要终端交互的程序，然后用不同方式照看它。

本文面向想理解 Wuu 工作方式的开发者。正文先讲真实行为和使用边界；具体工具名、参数和源码放在最后。

> 调查基线：2026-07-22。本文只描述当前已经实现的行为。未来的生命周期调整记录在[后台命令生命周期设计稿](../../plans/2026-07-21-background-command-lifecycle-design.md)中。
...
这三类任务都从同一个命令入口进入，也由同一个进程管理器记录。这样，一条起初看起来很快、后来实际运行很久的命令，可以直接转为后台任务，不需要重新执行。
...
到达等待上限时，Wuu 的首选行为不是杀掉任务，而是把已经运行的进程转成后台任务：

- 原进程继续运行，不会重跑；
- 超时前已经产生的输出不会丢失；
- Agent 会拿到一个任务编号，可以查看进度、写入输入或主动停止；
- Wuu 默认每 30 分钟检查一次被自动转后台的任务；
- 任务自然结束后，Wuu 会主动唤醒 Agent 继续工作。

只有在进程管理器或日志准备失败、无法安全接管时，Wuu 才会停止超时命令。因此，看到“等待超时”不能直接理解为“任务已结束”，还要看它是否已经转为后台任务。

## 明确启动一个后台任务

对开发服务器、watch 模式等已知长任务，Agent 应该从一开始就使用后台方式，而不是故意把普通命令的等待时间设得很短。
...
读取输出时，Wuu 默认给出最后 32 KiB。需要完整内容时可以按字节位置向后翻页。等待新输出是一次有上限的观察，不是永久占住 Agent：新输出会返回，任务结束会立即返回，最长等待 5 分钟。
...
- **继续工作：** 默认方式。任务自然结束后，Wuu 把退出状态和最后一段输出作为一条系统生成的消息送回所属对话，并开启新的 Agent 轮次。
- **静默结束：** 适合不应再次唤醒 Agent 的服务。任务仍可被查看和停止，但退出时不创建新轮次。

完成事件不会只存在内存里。进程管理器先把“这个结果还没交给 Agent”写入本地记录，再发送即时事件。如果即时事件因为队列已满而丢失，app-server 会重新扫描记录；对话恢复时也会补查。因此，事件通道只是加速通知，磁盘记录才是补偿依据。

定时复查使用同一套机制。到点后，Agent 收到当前状态和日志尾部，可以选择干预、调整下一次复查，或什么也不做。任务一旦结束，复查自动取消，最终完成通知接管。
...
当前桌面的任务面板不是接收一条独立的实时进程推送，而是大约每 1.5 秒向 app-server 查询一次；命令调用和完成续转仍通过正常的对话事件显示。因此，面板状态可能比实际进程晚一个轮询周期。
...
最后一点是当前实现的重要差异：两个入口使用相同的 Bash 语法和进程管理能力，但还没有完全共用同一条启动管线。
...
系统会分析命令是否只读、是否适合并行、是否可能读取密钥、是否包含破坏性 Git 操作、下载安装或外部修改。这些信息帮助调度、展示和边界判断，但 Bash 仍是通用解释器，静态分析不等于完整 sandbox。

工作区边界主要约束命令从哪个目录启动，并不把子进程关进操作系统沙箱。命令仍可能使用绝对路径、访问网络或启动其他程序；需要强隔离时，仍应使用容器、受限系统用户或操作系统提供的 sandbox。
...
- **随会话结束：** runtime 清理时停止仍在运行的任务；
- **跨 app-server 重启：** 进程可以继续存在，新 manager 通过 PID、进程组和启动身份重新观察它。

跨重启不等于完整接管。新 manager 不能重建原来的终端、stdin 或 Go 进程句柄，也可能拿不到可靠退出码。当前模型仍能创建这种长期任务，但设计方向是停止新增，把它收敛为旧记录兼容能力。Wuu 不准备成为带重启策略、健康检查和退避机制的通用服务 supervisor。

当前清理闭环还不完整：正常关闭 app-server 会清理“随会话结束”的任务，但删除一个 thread、关闭 subagent 或从缓存移除 thread，并不保证立即停止它们已经启动的所有任务；后台进程也不会让 Desktop 为它持续保活 app-server。这是现状缺口，不应被理解为可靠的 owner 生命周期管理。

## 当前最重要的限制

1. 普通命令和显式后台命令的启动环境还不完全一致。
2. 后台日志目前无界，缺少容量和保留治理。
...
7. 长期任务能力目前超过产品希望承诺的边界，生命周期仍待收敛。
