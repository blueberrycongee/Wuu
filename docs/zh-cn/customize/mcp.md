...
streamable HTTP；若初始化 POST 被 4xx 拒绝，再回退到 SSE。
...
当前 OAuth 是有限的桌面流程：只适用于远程 URL 服务器，需要在定义中加入 `oauth`，
并由用户手工完成授权码回填；没有对应的 CLI 登录命令或自动回调监听。至少提供回调
地址；已有客户端凭据时也可以一并填写：
...
确认配置了 `oauth.redirect_uri`，服务器公布了 OAuth discovery 元数据；动态注册失败时
改用服务方提供的 `client_id` 和 `client_secret`。
