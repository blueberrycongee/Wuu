...
| 7 | Change workbench layout and the complete theme language | Partial | Theme selection and generation cleanup work. One generated registry now keeps manifest, SDK, and Desktop token validation aligned, legacy names map to current semantic tokens, and contract tests require every current token to have a host consumer. A protected Layer Host and the shared Modal expose stable semantic DOM hooks. Completion still requires migrating the remaining menus, popovers, tooltips, notices, and foundational controls, then replacing the nominal pane placement API with a real constrained layout model and component recipes. |
